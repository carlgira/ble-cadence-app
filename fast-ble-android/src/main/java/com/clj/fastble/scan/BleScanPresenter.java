...
public abstract class BleScanPresenter implements BluetoothAdapter.LeScanCallback {
    ...
    private List<IBleDevice> mIBleDeviceList = new ArrayList<>();
        ...
        public void handleMessage(Message msg) {
            ...
            if (bleScanPresenter != null) {
                if (msg.what == BleMsg.MSG_SCAN_DEVICE) {
                    final IBleDevice bleDevice = (IBleDevice) msg.obj;
                    if (bleDevice != null) {
    ...
    private void handleResult(final IBleDevice bleDevice) {
    ...
    private void checkDevice(IBleDevice bleDevice) {
    ...
    private void correctDeviceAndNextStep(final IBleDevice bleDevice) {
        if (mNeedConnect) {
            ...
            for (IBleDevice result : mIBleDeviceList) {
    ...
    public final void notifyScanStopped() {
        ...
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                onScanFinished(mIBleDeviceList);
            }
        });
    }
    ...
    public abstract void onLeScan(IBleDevice bleDevice);

    public abstract void onScanning(IBleDevice bleDevice);

    public abstract void onScanFinished(List<IBleDevice> bleDeviceList);
}
